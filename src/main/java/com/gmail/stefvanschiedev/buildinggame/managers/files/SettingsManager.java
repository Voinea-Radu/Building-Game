...
public final class SettingsManager {
	...
	private YamlConfiguration vipCodes;
	...
	public void setup(Plugin p, boolean save) {
        ...
        vipCodesFile = new File(dataFolder, "vip-codes.json");
		...
		vipCodes = YamlConfiguration.loadConfiguration(vipCodesFile);
        ...
        if (!vipCodesFile.exists()) {
            try {
                if (!vipCodesFile.createNewFile())
                    logger.warning("Unable to create vip-codes file");
                save();
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    @NotNull
    @Contract(pure = true)
    public YamlConfiguration getVipCodes() {
        return vipCodes;
    }
