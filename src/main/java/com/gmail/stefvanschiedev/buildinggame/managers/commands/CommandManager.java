...
public class CommandManager extends BaseCommand {
    ...
    public void onJoinVip(Player player, String purchaseCode) {
        ...
        try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
            ...
            JsonObject responseObject = new Gson().fromJson(builder.toString(), JsonObject.class);
            JsonArray paymentObjects = responseObject.getAsJsonArray("payments");
            List<String> paymentTxnIDs = new ArrayList<>();
            ...
            for (String txnID : paymentTxnIDs) {
                if (purchaseCode.equals(txnID)) {
                    if (currentlyUsedCodes.contains(purchaseCode)) {
                        continue;
                    }
                    arena.join(player);
                    currentlyUsedCodes.add(purchaseCode);
                    SettingsManager.getInstance().getVipCodes().set("used-codes", currentlyUsedCodes);
                    return;
                }
            }
            MessageManager.getInstance().send(player, MESSAGES.getStringList("join.vip-code-not-existent"));
